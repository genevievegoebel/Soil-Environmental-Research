...
setwd("/Users/f0046hz/Desktop/Thesis/Data/Data by Site/Corinth/Soil/")
getwd()
...
procSoildat <- read.csv("2021 Corinth Soil Main Processing Data Sheet.csv", header=TRUE)
...
### Water-Extractable Aluminum Analysis ----
...
### Water-Extractable Iron Analysis ----
Fewathist <- ggplot(corinthSoilmeans, aes(Al.weighted.x_mean)) +
  geom_histogram(binwidth = 0.25,
                 col = "black",
                 aes(fill = ..count..)) +
  scale_fill_gradient("Count", low = "purple", high = "orange") +
...
#Normal quantile plot (NQP)
AlwatNQP <- ggplot(data = ironWaterrorinfo, aes(sample = resids)) + 
  geom_qq( ) + geom_qq_line( ) + 
  labs(x = "Theoretical", y = "Water-Extractable Al (ug/mL)") 
