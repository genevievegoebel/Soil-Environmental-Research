...
files <- list.files(path = "2019-9-4 SWaN", pattern = "*.csv", full.names = TRUE, recursive = FALSE)
...
begin <- as.POSIXct("2019-09-04 00:00:01")
...
processeddata <- read_csv("2019-9-4 Processed Data.csv", col_types = "dccffcdddd")
    ...
    for(p in 1:length(averages$index)) {
      ...
      for(q in 1:length(avgdupes)){
        ...
        if(processeddata$Time[o] == averages$timeindex[p]){
          #Checks to see if the average value has a unique timeindex
          if(averages$timeindex[p]!= avgdupes[q]){
            #Assigns processeddata entry at o with average value at p
            processeddata$FiboxAv[o] <- averages$fiboxaverage[p]
            #If there's more than one average value with the same timeindex, further measures must be taken...
          } else{
...
write.csv(test, "2019-9-4 Processed Standards Out.csv")
